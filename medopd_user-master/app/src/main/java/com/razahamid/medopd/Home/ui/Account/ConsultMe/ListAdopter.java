...
public class ListAdopter extends RecyclerView.Adapter<ListAdopter.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder holder,final int position) {
        DocumentSnapshot documentSnapshot=modelLists.get(position).documentSnapshot;
       holder.name.setText(documentSnapshot.getString(ref.FirstName));
       if (documentSnapshot.contains(ref.ImageUrl)){
           Glide.with(mContext).load(documentSnapshot.getString(ref.ImageUrl)).into(holder.imageView);
       }
       if (documentSnapshot.contains(ref.Education)){
           holder.degree.setText(documentSnapshot.getString(ref.Education)+"\n"+documentSnapshot.getString(ref.Experience)+" Years of Experience");
       }
       if (documentSnapshot.contains(ref.professionalHeading)){
           holder.headline.setText(documentSnapshot.getString(ref.professionalHeading));
       }
    }
    @Override
